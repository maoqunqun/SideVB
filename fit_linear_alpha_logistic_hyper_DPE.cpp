...
List fit_linear_alpha_hyper_DPE(
  ...
  // Initialize omega and logit_pi outside the DPE loop so they carry across steps
  // (matches R where side_omega and logit_pi are initialized once before the loop)
  arma::vec side_omega(p, fill::ones);
  side_omega *= 0.25;

  arma::rowvec logit_pi = side_mu.t() * Z.t();
  ...
  for (int step_idx = 0; step_idx < n_dpe_steps; step_idx++) {
    ...
    // Build DPE prior covariance for this step
    ...
    // Extract prior precision and prior precision * mean for side information
    // Always use the fixed original side_mu (prior mean never updated, matching R)
    arma::mat side_P = inv_sympd(side_Sigma_step);
    arma::vec side_Pmu = side_P * side_mu;
    ...
    if (verbose) {
      ...
      for (int qi = 0; qi < q; qi++) Rcout << theta_mean(qi) << " ";
      Rcout << std::endl;
      Rcout << "Theta posterior SEs:     ";
      for (int qi = 0; qi < q; qi++) Rcout << theta_se(qi) << " ";
      Rcout << std::endl;
    ...
    List res_step = List::create(
      ...
      Named("dpe_side_prior_mu") = side_mu,
